...
//			return CELL_OK;
		...
		dyingBiomass = new double[biomass.length]; //Portion of the biomass that will die
		...
		//Choosing which model to run (loops through all of them) - Master Loop
		for (int a=0; a<models.length; a++)
		{
			...
			//For the diffusion algorithm, there is a limit to biomass
			//If the biomass is zero, we're not going to do anything
			//If the biomass hits the maximum, you won't grow (it'll diffuse)
			if (biomass[i] == 0 || Utility.sum(biomass) >= cParams.getMaxSpaceBiomass())
			{
				...
				double[] exchFlux = ((FBAModel)models[i]).getExchangeFluxes(); //Exchange fluxes for the model
				double[] mediaDelta = new double[exchFlux.length]; //Change in media due to secretion/uptake
			...
			//Activation step - if it hasn't run before, it'll start running at random
			//Some bacteria are in a dormant stage until they activate and start growing - simulate the biological effect
			//Not dealing w bacteria so, does not matter as much
			...
			//If it has reached the limiting carrying capacity (stationary status), don't do anything
			...
			//Base bounds hard coded in the model file, but not calculated by Michaelis-Menten
			...
			{
				//Usually use the MONOD exchange style, calculates the lower bounds for uptakes based on Michaelis-Menten
				//Uses Vmax and Km
				case MONOD :
					...
					for (int j=0; j<lb[i].length; j++)
					{
						...
						// Lowerbounds greater than 0.0 indicating forced production (so we set the uptake to that)
						...
						//Get the parameter for Michaelis-Menten
						//Can set up the parameters for each reaction separately, or have a default one for all of them
						//If those are not set through the model file, we just use the default
						...
						//Calculate the rates of uptake only
						//media[j]/(cParams.getTimeStep()*biomass[i]) this is all the media we have
						//If the media present in the cell is not enough, we do a different caluculation
						...
						{
							//Take whatever media we have, lb might be hard coded in the model
							//Don't want to uptake more than we have
							rates[j] = Math.min(Math.abs(lb[i][j]),Math.abs(media[j]/(cParams.getTimeStep()*biomass[i])));
						}
						else //If we have enough, then we just calculate it directly
							//lb is the hard coded bounds (since we're doing uptake, they're all positive) we take the smaller uptake (limiting one)
			...
			/************************* Write lower bounds *********************/
			//Override the hard coded lower bounds with the ones we calculated earlier
			//Rho is just a parameter for debugging, just leave it there
			//Rho = 1 so it's trivial
			for (int j=0; j<lb[i].length; j++)
			{
			...
			//Set the exchange lower bounds
			((FBAModel)models[i]).setExchLowerBounds(lb[i]); // here is where the new bounds are set

			/************************* SET MAX BIOMASS *****************************/

			//Calculates what the upper bound for biomass growth is

			//only set if the upper bound due to space constraints is lower than the default UB
			//(cParams.getMaxSpaceBiomass() - (Utility.sum(biomass) + Utility.sum(deltaBiomass))) - how much it has left to grow
			...
			//Infeasible solution, set biomass to zero, don't do anything
			if (stat != 5 && stat != 180)
			{
				...
				//Changing the media based on those fluxes
...
//					System.out.print("\t" + exchFlux[j]);
				...
				//Genome cost = based on the total fluxes on the reaction, it calculates the cost to maintain the enzymes
				//Maintenance for the protein production

				//Make the change somewhere here and modify the media delta
				//Check when biomass delta is negative (i.e. net loss)
				...
				if (models[i].getMaintenance() != -1) {

					double maintenanceFlux = ((FBAModel) models[i]).getMaintenanceFluxSolution();

					if (maintenanceFlux < models[i].getMaintenanceFlux()) {
						deltaBiomass[i] -= biomass[i] * (1.0 - maintenanceFlux / models[i].getMaintenanceFlux());
					}
				}
...
//				System.out.println("solution: " + ((FBAModel)models[i]).getObjectiveSolution());
			...
			//Toxic related death, different from loss of resources
			//In this one, the death rate is just how much they consume
			//We need to coordinate this when we do this
			//Integrate ours into theirs after we have a completely working model
		...
		//If the cell is not stationary, this is where it uptakes the media, uses the FBA exchange fluxes from earlier
		if (stationaryStatus == false)
		{
			...
			//The problem is that if you have a lot of models, and one of them takes all of one nutrient
			//Maybe should have split equally based on the rates
			//This just allows one of them to take all
			//Maybe change this slightly so we're not duplicating code so much (maybe put them in a function)
